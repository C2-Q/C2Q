...
Expected input: python_programs.csv with columns:
...
def main(
    ...
    out_syntax_failures: str | None = None,
):
    csv_path = Path(csv_path)
    out_snippet_metrics = Path(out_snippet_metrics)
    out_family_summary = Path(out_family_summary)
    if out_syntax_failures is None:
        out_syntax_failures = str(csv_path.with_name("syntax_failures.csv"))
    out_syntax_failures = Path(out_syntax_failures)
    out_snippet_metrics.parent.mkdir(parents=True, exist_ok=True)
    out_family_summary.parent.mkdir(parents=True, exist_ok=True)
    out_syntax_failures.parent.mkdir(parents=True, exist_ok=True)
    ...
    bad[["labels", "family", "code_snippet", "code_clean"]].to_csv(out_syntax_failures, index=False)
    print(f"[debug] syntax_ok=False rows: {len(bad)}")
    print(f"[debug] dumped to: {out_syntax_failures}")
...
def resolve_input_csv(primary_csv: Path, backup_csv: Path) -> Path:
    ...
    if backup_csv.exists():
        print(f"[info] Primary CSV not found, using backup CSV: {backup_csv}")
        return backup_csv
...
def parse_args():
    ...
    default_csv = repo_root / "src" / "parser" / "python_programs.csv"
    default_backup_csv = repo_root / "src" / "parser" / "data.csv"
    default_snippet = repo_root / "src" / "parser" / "snippet_metrics.csv"
    default_family = repo_root / "src" / "parser" / "family_summary.csv"
    default_syntax = repo_root / "src" / "parser" / "syntax_failures.csv"

    parser = argparse.ArgumentParser(
        description="Implementation-level dataset validation for C2Q parser inputs."
    )
    parser.add_argument(
        "--csv-path",
    ...
    parser.add_argument(
        "--out-snippet-metrics",
        default=str(default_snippet),
        help="Output CSV path for per-snippet metrics.",
    )
    parser.add_argument(
        "--out-family-summary",
        default=str(default_family),
        help="Output CSV path for family-level summary.",
    )
    parser.add_argument(
        "--out-syntax-failures",
        default=str(default_syntax),
        help="Output CSV path for syntax-failing snippets.",
    )
...
if __name__ == "__main__":
    args = parse_args()
    input_csv = resolve_input_csv(Path(args.csv_path), Path(args.backup_csv_path))
    main(
