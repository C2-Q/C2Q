...
def resolve_model_dir(repo_root: Path) -> Path | None:
...
class MyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        # Lazy import to avoid hard failure when optional Qiskit stacks conflict in fresh envs.
        self.parser = None
        self.parser_available = False
        try:
            from src.parser.parser import Parser  # local import by design
    ...
    def test_diversity_direct_metrics(self):
        repo_root = Path(__file__).resolve().parents[2]
        default_primary_csv = repo_root / "src" / "parser" / "python_programs.csv"
        default_backup_csv = repo_root / "src" / "parser" / "data.csv"
        default_out_dir = repo_root / "src" / "tests" / "diversity_out"

        input_csv = resolve_input_csv(
            Path(os.getenv("C2Q_INPUT_CSV", str(default_primary_csv))),
            Path(os.getenv("C2Q_BACKUP_CSV", str(default_backup_csv))),
        )
        out_dir = Path(os.getenv("C2Q_DIVERSITY_OUT", str(default_out_dir)))
        os.makedirs(out_dir, exist_ok=True)

        per_instance = []
        failures = Counter()
        if not self.parser_available:
            failures["parser_unavailable_env"] += 1
        ...
        with open(input_csv, "r", encoding="utf-8") as f:
            ...
            for case_idx, row in enumerate(dr, start=1):
                try:
                    ...
                    if self.parser_available and self.parser is not None:
                        try:
                            parser_tag, data = self.parser.parse(clean_code)
                        except Exception:
                            parser_tag, data = None, None
                            failures["parser_parse_error"] += 1
                        ...
                        parser_tag, data = "Unavailable", None
        ...
        write_csv(str(out_dir / "metrics_per_instance.csv"), per_instance)

        failures_rows = [{"failure_type": k, "count": v} for k, v in sorted(failures.items())]
        write_csv(str(out_dir / "failures_summary.csv"), failures_rows, fieldnames=["failure_type", "count"])
        ...
        write_csv(str(out_dir / "raw_vs_kept_by_tag.csv"),
                  raw_vs_kept,
        ...
        write_csv(str(out_dir / "summary_by_tag.csv"), summary_rows)
        ...
        write_csv(str(out_dir / "uniqueness_by_tag.csv"), uniq_rows)
        ...
        write_csv(str(out_dir / "buckets_by_tag.csv"), bucket_rows,
                  fieldnames=["tag", "metric", "bucket", "count", "ratio"])

        # algorithm-family outputs
        write_csv(str(out_dir / "algorithm_family_per_instance.csv"),
                  algo_per_instance,
        ...
        write_csv(str(out_dir / "algorithm_family_by_tag.csv"),
                  dist_rows,
        ...
        write_csv(str(out_dir / "algorithm_diversity_summary.csv"),
                  summary_algo_rows,
                  fieldnames=["tag", "count", "n_families", "entropy", "top_family", "top_family_ratio"])

        # signal dump for auditing
        write_csv(str(out_dir / "algorithm_signals_per_instance.csv"),
                  algo_signals_rows)
