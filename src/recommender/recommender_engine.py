...
def recommender(qc):
    ...
    for index_p, provider in enumerate(provider_list):
        ...
        for index_d, device in enumerate(device_list):
            ...
            if device_dict["device_qubits"] >= qc.num_qubits:
                ...
                qc_transpiled = transpile(qc, device_dict["backend"], seed_transpiler=77, layout_method='sabre', routing_method='sabre')
                ...
                if device == "Quantinuum H1" or device == "Quantinuum H2":
                    ...
                    hqc = 5 + ((circuit_dict["gates"]["single_qubit"]+10*circuit_dict["gates"]["two_qubit"]+5*circuit_dict["gates"]["measurement"])*(num_shots/5000))
                ...
                if provider == "Azure Quantum":
                    ...
                    if device == "IonQ Aria":
                        price_iteration = (circuit_dict["gates"]["single_qubit"]*price_list_shots[index_d][0] + circuit_dict["gates"]["two_qubit"]*price_list_shots[index_d][1])*num_shots
                    ...
                    if device == "Quantinuum H1" or device == "Quantinuum H2":
                        price_iteration = 5+num_shots*((circuit_dict["gates"]["single_qubit"]+10*circuit_dict["gates"]["two_qubit"])/5000)
                ...
                print(f" - Time to execute on the quantum computer with {num_iterations} iterations and {num_shots} shots: {round(time_to_execute, 6)} seconds")
    ...
    print(f" - Lowest error: {min_error_device[0]} from {min_error_device[1]} with a calculated error of {round(min_error_device[2]*100, 2)}%, time to execute: {round(min_error_device[3], 6)} seconds and a price of ${round(min_error_device[4], 2)}.")
    print(f" - Lowest time: {min_time_device[0]} from {min_time_device[1]} with a calculated error of {round(min_time_device[2]*100, 2)}%, time to execute: {round(min_time_device[3], 6)} seconds and a price of ${round(min_time_device[4], 2)}.")
    print(f" - Lowest price: {min_price_device[0]} from {min_price_device[1]} with a calculated error of {round(min_price_device[2]*100, 2)}%, time to execute: {round(min_price_device[3], 6)} seconds and a price of ${round(min_price_device[4], 2)}.")
